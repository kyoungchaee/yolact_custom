...
from yolact_cpu import Yolact
#from utils.augmentations import BaseTransform, FastBaseTransform, Resize
from utils.augmentations_cpu import BaseTransform, FastBaseTransform, Resize
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            x1, y1, x2, y2 = boxes[j, :]
            color = tuple([int(x) for x in (get_color(j) * 255).cpu().numpy().astype(np.uint8)])
            score = scores[j]
...
def prep_coco_cats():
    ...
    # print(coco_cats[transformed_cat_id])

def get_coco_cat(transformed_cat_id):
    """ transformed_cat_id is [0,80) as indices in cfg.dataset.class_names """

    # print('transformed_Cat_id :' , transformed_cat_id)
    # print('coco_Cats :', coco_cats)
    # print(coco_cats[transformed_cat_id])
...
class Detections:
    ...
    def add_bbox(self, image_id:int, category_id:int, bbox:list, score:float):
        ...
        # print('add_bbox안의 category_id :',category_id )
    ...
    def add_mask(self, image_id:int, category_id:int, segmentation:np.ndarray, score:float):
        ...
        # print('add_mask안의 category_id :', category_id)
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    if is_webcam:
        ...
        num_of_frames = round(vid.get(cv2.CAP_PROP_FRAME_COUNT))
    else:
    ...
    def play_video():
        try:
            nonlocal frame_buffer, running, video_fps, is_webcam, frames_displayed, vid_done ##num_frames이 원래 여기서 nonlocal로 선언,
...
def evaluate(net:Yolact, dataset, train_mode=False): ##측정하는 함수
    ...
    if args.image is not None:
        ...
        if ':' in args.video:
            ...
            evalvideo(net, args.video) ##여기까지 실행됨은 알 수 있음
            #print("return 하고 나갈듯")
        # return
    # print(classes_list)
    # print(scores_list)
...
'''-------------------------------------
    ...
    print('dataset type은 무엇일까요 : ', type(dataset))
    ...
    # print('여기일까333, play_video 하고 나옴')
    try:
        # print('여기일까333-1')
        # Main eval loop
...
def to_json(num_frames, classes_list, boxes_list, scores_list) :
...
if __name__ == '__main__':
    ...
    with torch.no_grad(): ## 해당 블록을 히스토리 트래킹하지 않을 것
